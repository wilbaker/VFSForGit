...
namespace GVFS.FunctionalTests.Tests.EnlistmentPerFixture
{
    ...
    public class PrefetchVerbTests : TestsWithEnlistmentPerFixture
    {
        ...
        {
            int expectedCount = 2;
                string output = this.Enlistment.Prefetch($"--files {Path.Combine("GVFS", "GVFS", "Program.cs")};{Path.Combine("GVFS", "GVFS.FunctionalTests", "GVFS.FunctionalTests.csproj")} --hydrate");
            this.ExpectBlobCount(output, expectedCount);
            output.ShouldContain("Hydrated files:   " + expectedCount);
